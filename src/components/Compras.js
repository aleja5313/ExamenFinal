...
export const Compras = () => {
  ...
  useEffect(() => {
    ...
    fetch(url, settings)
      ...
      .then((json) => {
        ...
        const i = json.ingredients.map((ingrediente) => {
          return 1;
        });
        setCantidades(i);
      });
  ...
  return (
    <Container>
      ...
      <br />
      <h4>Items {cantidadItems}</h4>
      <h4>Subtotal {valorTotal} $</h4>
      <h4>Gastos de Envío 7,00 $</h4>
      <h2>Total  {(Math.round(valorTotal * 7 * 100) / 100).toFixed(2)} $</h2>
      <button onClick={()=>mostartAlerta()} >Comprar ingredientes: {(Math.round(valorTotal * 7 * 100) / 100).toFixed(2)} $</button>
